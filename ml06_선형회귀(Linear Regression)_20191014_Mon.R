...
# 데이터 준비
heights <- read.csv ( file ="mlwr/heights.csv")
...
plot(heights, col = rgb(0.7,0.2,0.2,1.0))
# rgb(red, green, blue, 불투명도)
abline(h = mean(heights$son))
# abline(): 보조선, h(horizontal): 수평 보조선, v(vertical): 수직 보조선
...
# Pearson's Correlation Coefficent (상관 계수)
cor(heights$father, heights$son)
# -1 <= 피어슨 상관 계수 <= 1
# 피어슨 상관 계수의 절대값이 1에 가까울 수록 상관 관계가 높고,
# 0에 가까울 수록 상관 관계가 낮다.

# 선형 회귀식에 해당하는 점들은 행렬로 변경할 수 있다.
...
summary(lm_launch)
a <- lm_launch$coefficients[1] # 선형 모델의 y절편
a
