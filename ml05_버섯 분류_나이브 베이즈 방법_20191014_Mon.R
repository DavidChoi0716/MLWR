rm (list = ls())
# 버섯 분류(식용/독버섯) - 나이브 베이즈 방법

# 1. 데이터 준비
getwd()
setwd("C:/dev/lab-r") 
mushroom <- read.csv(file = "mlwr/mushrooms.csv", stringsAsFactors = T, encoding = "UTF-8")
...
sample_count <- round(nrow(mushroom)*0.75)
 # nrow(데이터 프레임) : 데이터 프레임의 행(row) 갯수 
 # roudn(): 반올림

set.seed(123) # 같은 순서의 난수들을 발생시키기 위해서  # 의사난수(pseudo random number)
