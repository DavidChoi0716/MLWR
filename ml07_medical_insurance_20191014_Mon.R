...
# 대다수의 사람들의 의료비는 0~15,000 사이에 분포s
...
# 수치형 변수를 이진화
# bmi의 값이 30이상이면 1, 그렇지 않으면 0으로 변환
...
# 흡연 + 비만 
str(insurance)
ins_model2 <- lm(formula = expenses ~ age + sex + bmi + children + smoker + region + age2
                 + bmi30 + smoker*bmi30, data = insurance)
ins_model2
