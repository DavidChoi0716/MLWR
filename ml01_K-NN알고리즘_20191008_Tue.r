...
getwd()
setwd("C:/dev/lab-r")
wbcd <- read.csv(file = "data/wisc_bc_data.csv", stringsAsFactors = F)
wbcd
...
# 학습 데이터/테스트 데이터의 진단 정보를 가지고 있는 벡터
...
predict <- knn(train = wbcd_train, test = wbcd_test, cl = wbcd_train_label, k = 39)
CrossTable(wbcd_test_label, predict, prop.chisq = F)
...
predict <- knn(train = train_data, test = test_data, cl = train_label, k=21)
